...
def get_biggest_bar(f_cont):
    get_from_dict = lambda x: f_cont["features"][x]["properties"]["Attributes"]["SeatsCount"]
    return max(range(0,len(f_cont["features"])),key = get_from_dict)


def get_smallest_bar(f_cont):
    get_from_dict = lambda x: f_cont["features"][x]["properties"]["Attributes"]["SeatsCount"]
    return min(range(0,len(f_cont["features"])),key = get_from_dict)


def get_closest_bar(f_cont, latt, longt):
    get_from_dict = lambda x: sqrt((float(f_cont["features"][x]["geometry"]["coordinates"][0])-longt)**2+(float(f_cont["features"][x]["geometry"]["coordinates"][1])-latt)**2)
    return min(range(0,len(f_cont["features"])),key = get_from_dict)
